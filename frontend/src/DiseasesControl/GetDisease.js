import {
  ...
  export default function GetDisease(props) {
    ...
    const takePicture = async () => {
      if (cameraRef.current) {
        ...
        setClickButton(true)
        try {
          const response = await fetch("http://192.168.1.4:5000/predict/leaf", {
            method: "POST",
            body: formData,
            headers: {
          ...
          if (response.ok) {
            ...
            navigation.navigate("Result", {res: responseData,  imageUri: photo.uri})
          } else {
      ...
      if (!result.canceled) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            Alert.alert("Success", "Photo uploaded successfully");
            setClickButton(false)
            navigation.navigate("Result", {res: responseData,  imageUri: result.assets[0].uri})
          } else {
            Alert.alert("Error", "Failed to upload photo");
          }
        } catch (error) {
